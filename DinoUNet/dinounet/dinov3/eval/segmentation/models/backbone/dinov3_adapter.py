...
class SpatialPriorModule_v1(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False):
        super().__init__()
        self.with_cp = with_cp
        ...
        # self.fc1 = nn.Conv2d(inplanes, 64, kernel_size=1, stride=1, padding=0, bias=True)
        # self.fc2 = nn.Conv2d(2 * inplanes, 128, kernel_size=1, stride=1, padding=0, bias=True)
        # self.fc3 = nn.Conv2d(4 * inplanes, 256, kernel_size=1, stride=1, padding=0, bias=True)
        # self.fc4 = nn.Conv2d(4 * inplanes, 512, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        def _inner_forward(x):
            ...
            bs, dim, _, _ = c1.shape
            # c1 = c1.view(bs, dim, -1).transpose(1, 2)  # 4s
            c2 = c2.view(bs, 128, -1).transpose(1, 2)  # 8s
            c3 = c3.view(bs, 256, -1).transpose(1, 2)  # 16s
            c4 = c4.view(bs, 512, -1).transpose(1, 2)  # 32s
...
class DINOv3_Adapter_v1(nn.Module):
    def __init__(
        self,
        backbone,
        interaction_indexes=[9, 19, 29, 39],
        ...
        super(DINOv3_Adapter_v1, self).__init__()
        self.backbone = backbone
        # Important: we freeze the backbone
        self.backbone.requires_grad_(False)
        ...
        block_fn = InteractionBlockWithCls
        # self.level_embed = nn.Parameter(torch.zeros(3, embed_dim))
        self.level_embed = nn.ParameterList([nn.Parameter(torch.zeros(128)),
                            nn.Parameter(torch.zeros(256)),
                            nn.Parameter(torch.zeros(512)),
                            ])
        self.spm = SpatialPriorModule_v1(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False)
        self.inter_dim = [128,256,512,512]
        self.interactions = nn.Sequential(
            *[
                block_fn(
                    # dim=embed_dim,
                    dim=self.inter_dim[i],
                    num_heads=deform_num_heads,
                    n_points=n_points,
                    init_values=init_values,
                    drop_path=drop_path_rate,
        ...
        self.up = nn.ConvTranspose2d(128, 128, 2, 2)
        self.norm1 = nn.SyncBatchNorm(64)
        self.norm2 = nn.SyncBatchNorm(128)
        self.norm3 = nn.SyncBatchNorm(256)
        self.norm4 = nn.SyncBatchNorm(512)

        self.up.apply(self._init_weights)
        self.spm.apply(self._init_weights)
        self.interactions.apply(self._init_weights)
        self.apply(self._init_deform_weights)
        torch.nn.init.normal_(self.level_embed[0])
        torch.nn.init.normal_(self.level_embed[1])
        torch.nn.init.normal_(self.level_embed[2])
    ...
    def forward(self, x):
        ...
        #     #     W_toks,
        #     # )
        #     outs.append(c[i].transpose(1, 2).view(bs, dim, H_toks, W_toks).contiguous())
        outs.append(c[0].transpose(1, 2).view(bs, 128, H_c * 2, W_c * 2).contiguous())
        outs.append(c[1].transpose(1, 2).view(bs, 256, H_c, W_c).contiguous())
        outs.append(c[2].transpose(1, 2).view(bs, 512, H_c // 2, W_c // 2).contiguous())
        # Split & Reshape
        # c2 = c[:, 0 : c2.size(1), :]
        # c3 = c[:, c2.size(1) : c2.size(1) + c3.size(1), :]
        # c4 = c[:, c2.size(1) + c3.size(1) :, :]

        c2 = c2.transpose(1, 2).view(bs, 128, H_c * 2, W_c * 2).contiguous()
        c3 = c3.transpose(1, 2).view(bs, 256, H_c, W_c).contiguous()
        c4 = c4.transpose(1, 2).view(bs, 512, H_c // 2, W_c // 2).contiguous()
        c1 = self.conv11(self.up(c2)) + c1
...
class DINOv3_Adapter_without(nn.Module):
    def __init__(
        self,
        backbone,
        interaction_indexes=[9, 19, 29, 39],
        ...
        super(DINOv3_Adapter_without, self).__init__()
        self.backbone = backbone
        # Important: we freeze the backbone
        self.backbone.requires_grad_(False)
    ...
    def forward(self, x):
        ...
        if self.add_vit_feature:
            x1, x2, x3, x4 =  all_layers[0][0].transpose(1, 2).view(bs, dim, H_toks, W_toks).contiguous(), all_layers[1][0].transpose(1, 2).view(bs, dim, H_toks, W_toks).contiguous(), all_layers[2][0].transpose(1, 2).view(bs, dim, H_toks, W_toks).contiguous(), all_layers[3][0].transpose(1, 2).view(bs, dim, H_toks, W_toks).contiguous()
