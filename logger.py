...
try:
    ...
    from utils import get_project_path, mkdirs # sometimes, there is a name conflict for 'utils' then we will use 'uutils'
...
class Logger(object):
    ...
    def __init__(self, args):
        ...
        '''
        将self.print_git_status()注释以避免一下告警信息，注释：只能消除一行
    ...
    def set_up_dir(self):
        ...
        self.logplt_path  = pjoin(project_path, "plot")
        mkdirs(self.weights_path, self.gen_img_path, self.log_path,self.logplt_path, self.cache_path)
        # mkdirs(self.weights_path, self.gen_img_path, self.cache_path)
        self.logtxt = open(self.logtxt_path, "a+",encoding="utf-8")
        self.script_hist = open('.script_history', 'a+',encoding="utf-8") # save local script history, for convenience of check
