...
def conv2x2(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation_rate=1):
    if kernel_size == (1, 3, 3):
        return nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
                         padding=(1, 1), bias=False, dilation=dilation_rate)
    else:
        return nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, padding=1, dilation_rate=1):
        ...
        self.conv1 = conv2x2(inplanes, planes, kernel_size, stride, padding=padding)
        self.bn1 = nn.InstanceNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv2x2(planes, planes, kernel_size=kernel_size, padding=padding,
...
class PFAM(nn.Module):
    ...
    def __init__(self, 
        ...
        self.weight_conv  = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(rank*2, 2, 3, 1, 1),
                nn.Softmax(dim=1)
            )
            for _ in out_ch_list
        ])

        self.cross_channel_att = nn.ModuleList([
            ChannelAttention(rank)
            for _ in out_ch_list
        ])

        self.conv = nn.ModuleList([
            nn.Conv2d(rank*2, rank, kernel_size=1, bias=bias)
        for _ in out_ch_list
        ])
    ...
    def forward(self, x_list: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        for i, x in enumerate(x_list):
            ...
            concat_fm = torch.cat([z_shared, z_specific], dim=1)
            weight_map = self.weight_conv[i](concat_fm)
            z_modulated = torch.cat([
                z_shared * weight_map[:, 0, ...].unsqueeze(1),
                z_specific * weight_map[:, 1, ...].unsqueeze(1),
            ], dim=1)  # (1, 2*c, h, w, t)
            z_modulated = self.conv[i](z_modulated)
...
class DINOv3EncoderAdapter(nn.Module):
    def __init__(self,
        ...
        self.pfam = PFAM(in_ch, rank, target_channels,
...
class VPAMNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 dinov3_pretrained_path: str = "pretrain/checkpoints/dinov3_vits16_pretrain_lvd1689m-08c60483.pth",
...
#
#     parser.add_argument('--datasetid', type=int, default=4,
...
def get_VPAMNet(num_channels=3, num_classes=6, use_bn=False):
    dinov3_vits16_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),"pretrain","dinov3_vits16_pretrain_lvd1689m-08c60483.pth")
    model = VPAMNet(
        # DINO
        dinov3_pretrained_path=dinov3_vits16_path,
...
if __name__ == '__main__':
    model = VPAMNet(
        # DINO
        dinov3_pretrained_path="/pretrain/dinov3_vits16_pretrain_lvd1689m-08c60483.pth",
        dinov3_model_name="dinounet_s",
        # 输入输出
        input_channels=3,
        num_classes=6,
